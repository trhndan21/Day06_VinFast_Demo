...
4. **User (Lượt 4):** "Thế giá lăn bánh ở Hà Nội của nó là bao nhiêu?"
   - **Expectation (Rolling Triggered):** Hệ thống đạt mức `len(memory) == 3`, tự động thực hiện quá trình tóm tắt ở dưới ngầm. Câu trả lời của AI vẫn phải trúng phóc về chiếc VF 8 bản Pin cao nhất, chứng tỏ bản tóm tắt đã bảo toàn Fact thành công.
...
1. **User:** "Chính sách đền bù pin 100% khi xe VF 9 bị ngập nước trong siêu bão Yagi là bao nhiêu tiền?"
   ...
   - **Hành vi UI:** Ẩn hoàn toàn điểm yếu thiếu dữ kiện, hệ thống trực tiếp xuất hiện một Thẻ Cảnh Báo tinh tế: *"Anh/chị có muốn em kết nối với tư vấn viên VinFast để được hỗ trợ chuyên sâu hơn thông tin này không?"* kèm theo nút bấm **[📞 Gọi tư vấn viên]**.
...
## Kịch bản 6: Kiểm thử Hủy Form Bật Lên (Cancel Booking)
*Mục đích: Đảm bảo UI hoạt động linh hoạt, cho phép người dùng đổi ý thoát khỏi luồng Đặt lịch.*

1. **User Action:** Kích hoạt Form bằng cách bấm "[Gọi tư vấn viên]".
2. **UI Action:** Hiện Form yêu cầu nhập Tên và Số điện thoại.
3. **User Action:** Bấm nút **"Hủy bỏ"** ở cuối Form.
4. **Expectation:** UI ẩn Form lập tức, trả lại giao diện Chat bình thường với thông báo "Đã hủy thao tác!". AI không bị treo.
...
1. **User Action:** Tại Sidebar, bấm chọn link **"🔑 Trang Quản Trị"** để di chuyển sang môi trường Admin.
...
4. **Expectation:** Một tệp `training_data.jsonl` được tải xuống máy chuẩn định dạng JSON, mở ra sẽ thấy đầy đủ các cột `{query, answer, label="..."}` sẵn sàng đút cho OpenAI huấn luyện bản GPT tiếp theo.
